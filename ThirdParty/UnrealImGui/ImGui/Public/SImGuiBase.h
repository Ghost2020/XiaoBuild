...
class IMGUI_API SImGuiBase : public SCompoundWidget
{
	typedef SCompoundWidget Super;

public:

	SLATE_BEGIN_ARGS(SImGuiBase){}
...
private:
	...
	void UpdateTransparentMouseInput(const FGeometry& AllottedGeometry) const;
	void HandleWindowFocusLost();

	void UpdateCanvasSize();

	void UpdateCanvasControlMode(const FInputEvent& InputEvent) const;
	...
	TWeakObjectPtr<UImGuiInputHandler> InputHandler = nullptr;
	...
	bool bTransparentMouseInput = true;
	bool bAdaptiveCanvasSize = true;
	bool bUpdateCanvasSize = true;
	bool bCanvasControlEnabled = true;

	TSharedPtr<SImGuiCanvasControl> CanvasControlWidget = nullptr;
	TWeakPtr<SWidget> PreviousUserFocusedWidget = nullptr;
};
