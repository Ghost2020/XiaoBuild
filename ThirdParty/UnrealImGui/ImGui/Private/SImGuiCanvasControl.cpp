...
FReply SImGuiCanvasControl::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	if (Operation.IsValid())
	{
		...
		const auto TempVec = ScreenToWidget.TransformVector(DragDropEvent.GetScreenSpacePosition() - Operation->StartPosition);
		const FVector2D DragDelta(TempVec.X, TempVec.Y);
