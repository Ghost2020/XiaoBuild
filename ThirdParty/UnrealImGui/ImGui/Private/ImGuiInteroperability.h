...
namespace ImGuiInterops
{
	...
	FORCEINLINE FColor UnpackImU32Color(const ImU32 Color)
	{
		// We use IM_COL32_R/G/B/A_SHIFT macros to support different ImGui configurations.
		return FColor{ static_cast<uint8>((Color >> IM_COL32_R_SHIFT) & 0xFF), static_cast<uint8>((Color >> IM_COL32_G_SHIFT) & 0xFF),
			static_cast<uint8>((Color >> IM_COL32_B_SHIFT) & 0xFF), static_cast<uint8>((Color >> IM_COL32_A_SHIFT) & 0xFF) };
	}
