...
const static FText LicenseAgreement = LOCTEXT("LicenseAgreement_Text", "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tXiaoBuild License Agreement\n"
	...
	"\tvIt is hereby acknowledged, that upon your use, download or installation of freethe Software Product, the"
	...
	"XiaoBuild from further collecting new Telemetry Data by :\n"
	"\t(i) if using the Free License or the"
	"Evaluation License, uninstalling the Free version of the Software, or\n"
	"\t(ii) if using s Subscription License,"
	...
	"any third party to use the Products in any manner;\n"
	"\t(ii)reverse engineer, decompile, disassemble or "
	"otherwise reduce to human - perceivable form the Products’ source code;\n" 
	"\t(iii)modify, revise, enhance\n"
	"or alter the Products;\n" 
	"\t(iv)copy or allow copies of the Products to be made except as provided under this Agreement;\n"
	"\t(v)use any backup or archival copies of the Products or any part thereof, or allow any"
	"third party to use such copies, for any purpose other than to replace an original copy in the event of the"
	"destruction of any Product components, if a Productbecomes defective;\n" 
	"\t(vi)place any Product onto a"
	"server other than the designated server or otherwise make suchProduct accessible to other users or"
	"the public;\n" 
	"\t(vii)use aProduct to provide third parties with managed services or any other services"
	"whether or not in return for remuneration of any kind;\n"
	"\t(viii)disclose the results of any testing or"
	"benchmarking of a Product to any third party;\n" 
	"\t(ix)remove or alter any trademarks or other proprietary"
	"right notices displayed on or in a Product;\n"
	"\t(x)circumvent, disable or otherwise interfere with security - "
	"related features of a Product or features that enforce use limitations;\t" 
	"\t(xi) export, make available or use"
	"a Product in any manner prohibited by applicable laws; and /or\n"
	"\t(xii)store or transmit any malicious"
	...
	"The parties agree that to the maximum extent permitted by law :\n"
	"\t(a)XiaoBuild is merely utilizing the"
	"Licensee's Cloud and is not providing the Cloud itself;\n"
	"\t(b) XiaoBuild bears no responsibility or liability "
