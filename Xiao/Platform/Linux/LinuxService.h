...
public:
	...
	static bool ServiceControl(const int32 ArgC, TCHAR* ArgV[])
	{
		...
		if (FParse::Param(*CmdLine, *SInstall))
		{
			...
			return FLinuxService::DoInstallService(ServiceDesc);
		}
		if (FParse::Param(*CmdLine, *SQuery))
		{
			QUERY_SERVICE_CONFIGW ServiceConfig;
			return FLinuxService::DoQueryService(GServiceName, ServiceConfig);
		}
		if (FParse::Param(*CmdLine, *SEnable))
		{
			return FLinuxService::DoEnableService(GServiceName);
		}
		if (FParse::Param(*CmdLine, *SDisable))
		{
			return FLinuxService::DoDisableService(GServiceName);
		}
		if (FParse::Param(*CmdLine, *SDelete))
		{
			return FLinuxService::DoDeleteService(GServiceName);
		}
		return false;
	}
};