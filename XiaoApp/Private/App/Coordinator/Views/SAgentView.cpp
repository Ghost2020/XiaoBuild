				...
				[
					...
					[
						...
						.HeaderRow(
							...
							// + SHeaderRow::Column(S_ColumnIdAvaGpu).Visibility(EVisibility::Collapsed).DefaultLabel(LOCTEXT("AvaGpu_Text", "GPU%")).DefaultTooltip(LOCTEXT("AvaGpu_ToopTip", "此时此刻GPU利用率")).FillSized(120.0).InitialSortMode(EColumnSortMode::Ascending).OnSort_Raw(this, &SAgentView::OnTableSort).SortMode_Raw(this, &SAgentView::GetSortModeForColumn, S_ColumnIdAvaGpu).HAlignHeader(HAlign_Center).VAlignHeader(VAlign_Center)
