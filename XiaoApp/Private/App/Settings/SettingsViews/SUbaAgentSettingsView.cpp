	...
	[
		...
		[
			...
			[
				...
				V_ADD_SPINBOX(MulcpuSpinBox, SModifiedAgentSettings.UbaAgent.Mulcpu, 1, 16, LOCTEXT("Mulcpu_Text", "该值与cpu数量相乘，得出最大cpu。默认为1"), FText::GetEmpty())
				V_ADD_SPINBOX(MaxConSpinBox, SModifiedAgentSettings.UbaAgent.MaxCon, 1, FPlatformMisc::NumberOfCoresIncludingHyperthreads(), LOCTEXT("MaxCon_Text", "代理可以接受的最大连接数"), FText::GetEmpty())
				V_ADD_SPINBOX(CapacitySpinBox, SModifiedAgentSettings.UbaAgent.Capacity, 1, 1024 * 1024, LOCTEXT("Capacity_Text", "本地存储能力"), FText::GetEmpty())
				V_ADD_SPINBOX(SendSizeSpinBox, SModifiedAgentSettings.UbaAgent.SendSize, (256 * 1024), (1024 * 1024), LOCTEXT("SendSize_Text", "从客户端发送到服务器的消息的最大大小"), FText::GetEmpty())
				...
				V_ADD_CHECKBOX(LogCheckBox, SModifiedAgentSettings.UbaAgent.bLog, LOCTEXT("Log_Text", "将所有绕过信息的进程记录到文件中（仅适用于调试版本）"))
				V_ADD_CHECKBOX(NoCustoMallocCheckBox, SModifiedAgentSettings.UbaAgent.bNoCustoMalloc, LOCTEXT("NoCustoMalloc_Text", "禁用进程的自定义分配器。如果你看到奇怪的碰撞，可以进行测试"))
				V_ADD_CHECKBOX(StoreRawCheckBox, SModifiedAgentSettings.UbaAgent.bStoreRaw, LOCTEXT("bStoreRaw_Text", "禁用存储压缩。这将使用更多的存储空间，并可能提高性能"))
				V_ADD_CHECKBOX(SendRawCheckBox, SModifiedAgentSettings.UbaAgent.bSendRaw, LOCTEXT("bSendRaw_Text", "禁用发送压缩。这将使用更多的带宽，但占用更少的cpu"))
				V_ADD_CHECKBOX(NoPollCheckBox, SModifiedAgentSettings.UbaAgent.bNoPoll, LOCTEXT("bNoPoll_Text", "No Poll"))
				V_ADD_CHECKBOX(NoStoreCheckBox, SModifiedAgentSettings.UbaAgent.bNoStore, LOCTEXT("bNoStore_Text", "不使用存储来存储文件（二进制文件等少数例外）"))
				V_ADD_CHECKBOX(ResetStoreCheckBox, SModifiedAgentSettings.UbaAgent.bResetStore, LOCTEXT("bResetStore_Text", "删除所有cas"))
				V_ADD_CHECKBOX(QuietCheckBox, SModifiedAgentSettings.UbaAgent.bQuiet, LOCTEXT("bQuiet_Text", "不输出任何登录控制台"))
				V_ADD_CHECKBOX(BinaryVersionCheckBox, SModifiedAgentSettings.UbaAgent.bBinaryVersion, LOCTEXT("bBinaryVersion_Text", "将使用二进制文件作为版本。这将导致每次主机端二进制文件更改时都会进行更新"))
				V_ADD_CHECKBOX(SummaryCheckBox, SModifiedAgentSettings.UbaAgent.bSummary, LOCTEXT("SummaryCheckBox_Text", "在会话结束时打印摘要"))
				V_ADD_CHECKBOX(SentryCheckBox, SModifiedAgentSettings.UbaAgent.bSentry, LOCTEXT("bSentryCheckBox_Text", "启用哨兵"))
				V_ADD_CHECKBOX(NoProxyCheckBox, SModifiedAgentSettings.UbaAgent.bNoProxy, LOCTEXT("NoProxyCheckBox_Text", "不允许此代理作为其他代理的存储代理"))
				V_ADD_CHECKBOX(KillRandomCheckBox, SModifiedAgentSettings.UbaAgent.bKillRandom, LOCTEXT("KillRandom_Text", "杀死随机进程并退出会话"))
