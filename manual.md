...
- [XiaoBuild](#xiaobuild)
  ...
  - [使用说明](#使用说明)
    ...
    - [五.代理设置工具](#五代理设置工具)
  - [常见问题](#常见问题)
    - [启动项目之后卡在%45编译材质，久久进度没有变化](#启动项目之后卡在45编译材质久久进度没有变化)
    - [通过构建进度查看工具会发现，又是代理会超过系统设置的最大同时能运行的进程数](#通过构建进度查看工具会发现又是代理会超过系统设置的最大同时能运行的进程数)
  - [更新日志](#更新日志)
    - [v1.0.0](#v100)
  - [许可协议](#许可协议)
  - [联系我们](#联系我们)
...
`代理`::将会注册安装代理服务，可以发起联合编译任务，同时也作为代理接受其他发起者的联合编译请求

`网络`::在代理安装的基础上还会额外注册调度器服务，调度器服务用于同步所有代理的各种数据，在此数据基础上用于分发构建任务。
...
### 启动项目之后卡在%45编译材质，久久进度没有变化

在这种情况下是XiaoScheduler这个程序出现异常崩溃了，重启就能解决这样的问题

### 通过构建进度查看工具会发现，又是代理会超过系统设置的最大同时能运行的进程数
